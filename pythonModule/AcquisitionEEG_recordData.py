...
# 5 : write theta, alpha, beta and gamma values within an text file
# 6 : write normalised theta, alpha, beta and gamma values (in [0,1]) within an text file
...
if __name__ == "__main__":
    ...
    # ouverture du ficher pour stocker les futurs messages UDP
    record4udp=open("record4udp.txt","w");
    # ouverture du fichier pour stocker les futurs messages OSC
    record4osc=open("record4osc.txt","w");
    ...
    try:
        while True:
            ...
            if now-lasttime > udp_period :
                ...
                # print nt, 'write record4udp at', now-inittime, ':'
                mystr='Theta;'+';'.join('%.5g' % val for val in thetaWaves)
                record4udp.write(mystr+"\n")
                ...
                # print nt, 'write record4osc at', now-inittime, ':'
                ...
                # then build the osc message to write
                mystr = ('Theta;'+';'.join('%.5g' % val for val in thetaWaves)) + ';' + \
                        ('Alpha;'+';'.join('%.5g' % val for val in alphaWaves)) + ';' + \
                        ('Beta;'+';'.join('%.5g' % val for val in betaWaves)) + ';' + \
                        ('Gamma;'+';'.join('%.5g' % val for val in gammaWaves))
                record4osc.write(mystr+"\n");
                lasttime=now
